...
const Promotions: React.FC<PromotionsProps> = ({ currentUser }) => {
    ...
    return (
        <main className="flex-1 p-6 bg-app-bg overflow-y-auto page-fade-in">
            <h1 className="text-2xl font-bold text-app-text-primary">Promotional Image Generator</h1>
            <p className="text-app-text-secondary mt-1">Create stunning promotional images for Indian festivals and offers, featuring your pharmacy's logo.</p>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
                <Card className="p-6 space-y-4">
                    <div>
                        <label htmlFor="prompt" className="block text-sm font-medium text-app-text-secondary mb-2">
                            Describe your promotion
                        </label>
                        <textarea
                            id="prompt"
                            rows={4}
                            className="w-full p-2 border border-app-border rounded-lg shadow-sm focus:ring-primary focus:border-primary bg-input-bg"
                    ...
                    <div>
                        <p className="text-sm font-medium text-app-text-secondary mb-2">Or try an example:</p>
                        <div className="flex flex-wrap gap-2">
                            {examplePrompts.map((p, i) => (
                                <button key={i} onClick={() => setPrompt(p)} className="px-3 py-1 text-xs text-app-text-secondary bg-hover border border-app-border rounded-full hover:opacity-80">
                                    {p}
                                </button>
                            ))}
                        </div>
                    ...
                    <button
                        ...
                        className="w-full px-4 py-3 text-base font-semibold text-white bg-primary rounded-lg shadow-sm hover:bg-primary-dark disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center justify-center"
                    >
                        {isLoading ? (
                            <>
                ...
                <Card className="p-6 flex flex-col items-center justify-center">
                    <div className="w-full aspect-square bg-hover rounded-lg flex items-center justify-center border-2 border-dashed border-app-border">
                        {isLoading && (
                        ...
                        {!isLoading && !generatedImageUrl && !error && (
                            <div className="text-center text-app-text-secondary">
                                <svg xmlns="http://www.w3.org/2000/svg" className="mx-auto h-12 w-12 text-app-text-tertiary" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14" /></svg>
